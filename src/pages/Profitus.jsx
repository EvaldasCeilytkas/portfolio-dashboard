...
function ProfitusProject({ data, projectId }) {
    ...
    return (
      <main className="re-page">
        <section className="re-card re-not-found">
          ...
          <p>Patikrink projekto nuorodą arba grįžk į Profitus portfelį.</p>
          <Link className="re-back" to="/platforms/profitus">
            ← Grįžti į Profitus
          </Link>
        </section>
      </main>
...
export default function Profitus() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const response = await fetch(`${import.meta.env.BASE_URL}data/profitus.json`);
        if (!response.ok) {
          throw new Error("Nepavyko užkrauti profitus.json failo.");
        }
        const result = await response.json();
        if (active) setData(result);
      } catch (loadError) {
        console.error(loadError);
        if (active) setError(loadError.message || "Nepavyko užkrauti Profitus duomenų.");
      } finally {
  ...
  if (loading) {
    return <main className="re-page"><div className="re-loading">Kraunami Profitus duomenys...</div></main>;
  }

  if (error || !data) {
    return <main className="re-page"><div className="re-not-found">{error || "Profitus duomenų nėra."}</div></main>;
  }

  return projectId ? (
    <ProfitusProject data={data} projectId={projectId} />
  ) : (

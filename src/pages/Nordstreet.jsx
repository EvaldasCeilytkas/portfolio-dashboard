...
function NordstreetProject({ data, projectId }) {
    ...
    return (
      <main className="re-page">
        <section className="re-card re-not-found">
          ...
          <p>Patikrink projekto nuorodą arba grįžk į Nordstreet portfelį.</p>
          <Link className="re-back" to="/platforms/nordstreet">
            ← Grįžti į Nordstreet
          </Link>
        </section>
      </main>
    ...
    <main className="re-page">
      <section className="re-project-hero">
        <div>
          ...
          <small>{project.loanCode || "NORDSTREET PROJEKTAS"}</small>
          <h1>{project.name}</h1>
...
export default function Nordstreet() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const response = await fetch(`${import.meta.env.BASE_URL}data/nordstreet.json`);
        if (!response.ok) {
          throw new Error("Nepavyko užkrauti nordstreet.json failo.");
        }
        const result = await response.json();
        if (active) setData(result);
      } catch (loadError) {
        console.error(loadError);
        if (active) setError(loadError.message || "Nepavyko užkrauti Nordstreet duomenų.");
      } finally {
  ...
  if (loading) {
    return <main className="re-page"><div className="re-loading">Kraunami Nordstreet duomenys...</div></main>;
  }

  if (error || !data) {
    return <main className="re-page"><div className="re-not-found">{error || "Nordstreet duomenų nėra."}</div></main>;
  }

  return projectId ? (
    <NordstreetProject data={data} projectId={projectId} />
  ) : (

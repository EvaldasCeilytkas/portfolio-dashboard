...
export default function GoalsPage() {
  ...
  return <section className="goals-center">
    ...
    <div className="gc-kpi-grid"><article className="gc-kpi is-blue"><span>Tikslo progresas</span><strong>{number(model.goalProgress)} %</strong><p>{model.targetDate ? `Prognozė: ${dateFormatter.format(model.targetDate)}` : "Tikslas nepasiekiamas"}</p></article><article className="gc-kpi is-green"><span>Prognozė po 5 metų</span><strong>{money(model.projections.find((item) => item.years === 5)?.value)}</strong><p>Su reguliariomis {money(settings.monthlyContribution)} įmokomis</p></article><article className="gc-kpi is-violet"><span>Pasyvios pajamos dabar</span><strong>{money(model.currentMonthlyIncome, 2)} / mėn.</strong><p>Pagal {number(settings.withdrawalRate)} % išėmimo taisyklę</p></article><article className="gc-kpi is-orange"><span>FIRE tikslas</span><strong>{money(model.fireTarget)}</strong><p>{number(model.fireProgress)} % finansinės laisvės kelio</p></article></div>

    <div className="gc-main-grid"><article className="gc-card gc-projection-card"><header><div><span className="gc-eyebrow">FORECAST</span><h3>Portfelio augimo scenarijai</h3></div><button type="button" className="gc-settings-button" onClick={() => setEditing(true)}>Keisti tikslus</button></header><ProjectionChart currentValue={model.currentValue} monthlyContribution={settings.monthlyContribution} expectedReturn={settings.expectedReturn} targetValue={settings.targetValue} /><div className="gc-projection-grid">{model.projections.map((item) => <div key={item.years}><span>Po {item.years} metų</span><strong>{money(item.value)}</strong></div>)}</div></article>
    <article className="gc-card gc-fire-card"><span className="gc-eyebrow">FIRE CENTER</span><h3>Finansinės laisvės progresas</h3><div className="gc-fire-ring"><ProgressRing value={model.fireProgress} label="FIRE" /></div><div className="gc-fire-row"><span>Norimos pajamos</span><strong>{money(settings.monthlyPassiveIncome)} / mėn.</strong></div><div className="gc-fire-row"><span>Dabartinis potencialas</span><strong>{money(model.currentMonthlyIncome, 2)} / mėn.</strong></div><div className="gc-fire-progress"><i style={{ width: `${clamp(model.fireProgress, 0, 100)}%` }} /></div><div className="gc-fire-boosts">{model.contributionScenarios.map((item) => <div key={item.extra}><span>+{money(item.extra)} / mėn.</span><strong>{duration(item.months)}</strong></div>)}</div><p>Kaip keistųsi pagrindinio tikslo terminas padidinus mėnesinę įmoką.</p></article></div>

    <article className="gc-card gc-simulator"><header><div><span className="gc-eyebrow">GOAL SIMULATOR</span><h3>Žaiskite su savo planu</h3></div><div className="gc-sim-result"><strong>{simulation.date ? dateFormatter.format(simulation.date) : "—"}</strong><span>{duration(simulation.months)} iki tikslo</span></div></header><div className="gc-sim-grid">
    ...
    ].map(([key, label, step]) => <label key={key}>{label}<input type="number" min="0" step={step} value={draft[key]} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} /></label>)}<footer><button type="button" className="is-secondary" onClick={() => setEditing(false)}>Atšaukti</button><button type="submit">Išsaugoti tikslus</button></footer></form></div>}
  </section>;
}

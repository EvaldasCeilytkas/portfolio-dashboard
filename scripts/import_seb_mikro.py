...
DEFAULT_INPUT = PROJECT_ROOT / "excel" / "SEB Mikro.xlsx"
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "public" / "data" / "platforms" / "seb-mikro.json"
)
            ...
            "holdings": [
                {
                    ...
                    "weight": (
                        round(item["currentValue"] / current_value * 100, 4)
...
def validate_document(document: dict[str, Any]) -> None:
    ...
    latest_custody_fee = finite_number(
        document["history"][-1].get("custodyFees")
    )
    # Apžvalgos portfelio vertėje einamojo mėnesio laikymo mokestis
    # atimamas iš bendros aktyvių ETF rinkos vertės.
    expected_net_value = rounded(active_value - max(latest_custody_fee, 0.0))
    if abs(expected_net_value - summary["currentValue"]) > 0.05:
        errors.append(
            "Aktyvių pozicijų grynoji vertė nesutampa su Apžvalgos verte: "
            f"{expected_net_value:.2f} != {summary['currentValue']:.2f}"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SEB Mikro Excel importeris į Portfolio V2 JSON."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def main() -> None:
    ...
    print("=" * 64)
    print("SEB MIKRO IMPORTER V1 — BROKERIO ETALONAS")
    print("=" * 64)

...
LOAN_SHEET_RE = re.compile(r"^[A-Z]{2}\d{10}$", re.IGNORECASE)
...
def find_input(project_root: Path, explicit: str | None) -> Path:
    ...
    preferred = [
        excel_dir / "Debitum.xlsx",
        project_root / "Debitum.xlsx",
    ]
    for path in preferred:
        if path.is_file():
            return path

    candidates = sorted(excel_dir.glob("Debitum*.xlsx")) if excel_dir.exists() else []
    if candidates:
        return candidates[-1]

    raise FileNotFoundError(
        "Nerastas Debitum Excel failas. Įdėk jį į excel/Debitum.xlsx "
        "arba naudok --input."
    )
...
def load_debitum(excel_file: Path) -> dict[str, Any]:
    ...
    try:
        ...
        data = {
            ...
            "platform": {
                ...
                "website": "https://debitum.investments",
            },
            "summary": summary,
            "history": history,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Debitum Excel → Portfolio V2 platformos JSON."
    )
    parser.add_argument("--input", help="Debitum Excel failo kelias.")
    parser.add_argument("--output", help="Debitum JSON išvesties kelias.")
    ...
    output_file = (
        ...
        if args.output
        else project_root / "public" / "data" / "platforms" / "debitum.json"
    )

    print("=" * 64)
    print("DEBITUM IMPORTER V1")
    print("=" * 64)

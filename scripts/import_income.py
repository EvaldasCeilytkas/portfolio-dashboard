...
def find_input(project_root: Path, explicit: str | None) -> Path:
    ...
    for path in (excel_dir / "Income.xlsx", project_root / "Income.xlsx"):
        if path.is_file():
            return path

    candidates = sorted(excel_dir.glob("Income*.xlsx")) if excel_dir.exists() else []
    if candidates:
        return candidates[-1]

    raise FileNotFoundError(
        "Nerastas Income Excel failas. Įdėk jį į excel/Income.xlsx "
        "arba naudok --input."
    )
...
def load_income(excel_file: Path) -> dict[str, Any]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Income Excel → Portfolio V2 platformos JSON."
    )
    parser.add_argument("--input", help="Income Excel failo kelias.")
    parser.add_argument("--output", help="Income JSON išvesties kelias.")
    ...
    output_file = (
        ...
        if args.output
        else project_root / "public" / "data" / "platforms" / "income.json"
    )

    print("=" * 64)
    print("INCOME IMPORTER V1")
    print("=" * 64)
    print(f"Excel failas: {excel_file}")

    data = load_income(excel_file)

...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    target_name = normalize_header("Income")
    ...
    if not candidate_starts:
        raise ValueError(
            f"Faile {path.name} nerastas tikras Income "
    ...
    if selected_col is None or selected_score <= 0:
        raise ValueError(
            f"Faile {path.name} rastas Income blokas, "
    ...
    for row in range(3, ws.max_row + 1):
        ...
        invested = round_money(ws.cell(row, selected_col).value)
        value = round_money(ws.cell(row, selected_col + 1).value)
    ...
    if not history:
        raise ValueError(
            f"Faile {path.name} nerasta Income mėnesinės istorijos."
        )

    print(
        f"Income pagrindinio failo stulpeliai: "
...
def build_platform(excel_path: Path) -> dict[str, Any]:
    ...
    try:
        platform_history = read_main_history(main_excel_path)
    except ValueError:
        ...
        print(
            "Income mėnesinės istorijos pagrindiniame faile dar nėra. "

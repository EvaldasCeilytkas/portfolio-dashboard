...
EXCEL_PATH = ROOT / "excel" / "Nectaro.xlsx"
...
PREVIEW_PATH = ROOT / "public" / "data" / "nectaro-platform.json"
...
def read_loan(ws, today: date) -> dict[str, Any]:
    ...
    invested = money(find_value(ws, "Investuota, Eur"))
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    history = read_main_history(main_excel_path, "Nectaro")

    if not history:
        raise ValueError(
            f"Faile {main_excel_path.name} nerasta Nectaro mėnesinė istorija. "
            "Patikrink Investavimas lapo Nectaro antraštę ir stulpelius."
        )
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            ...
            if item.get("slug") == "nectaro"
            or str(item.get("name", "")).strip().lower() == "nectaro"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-nectaro.json")
    ...
    print(f"Nectaro platforma {action}.")
    ...
    print("Atidaryk: http://localhost:5173/platforms/nectaro")

...
def find_input(project_root: Path, explicit: str | None) -> Path:
    ...
    for path in (excel_dir / "Lande.xlsx", project_root / "Lande.xlsx"):
        if path.is_file():
            return path

    candidates = sorted(excel_dir.glob("Lande*.xlsx")) if excel_dir.exists() else []
    if candidates:
        return candidates[-1]

    raise FileNotFoundError(
        "Nerastas Lande Excel failas. Įdėk jį į excel/Lande.xlsx "
        "arba naudok --input."
    )
...
def read_overview(ws, epoch: datetime) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in range(3, ws.max_row + 1):
        loan_name = str(ws.cell(row, 3).value or "").strip()
        if not loan_name:
            continue

        loan_code = loan_name.rsplit("-", 1)[-1].strip()
        rows.append({
            ...
            "collateral": str(ws.cell(row, 5).value or "").strip() or None,
    ...
    summary = {
        ...
        "currentPrincipal": rounded(ws["R3"].value),
...
def read_loan_sheet(ws, epoch: datetime, today: date) -> dict[str, Any]:
    ...
    full_name = str(ws["B1"].value or "").strip()
    loan_code = ws.title.strip()
    country = str(ws["B2"].value or "").strip() or None
    borrower = str(ws["B3"].value or "").strip() or None
    collateral = str(ws["B4"].value or "").strip() or None
    ltv_raw = ws["B5"].value
    ltv = (
        rounded(finite_number(ltv_raw) * 100, 4)
        if ltv_raw not in (None, "")
        else None
    )
    ...
    xirr_raw = ws["N3"].value
    xirr = (
        rounded(finite_number(xirr_raw) * 100, 4)
        if xirr_raw not in (None, "")
        else None
    )
    actual_end = parse_date(ws["O3"].value, epoch)
    sheet_delay = int(finite_number(ws["P3"].value))
    ...
    completed = actual_end is not None or (
        invested > EPSILON and principal_returned >= invested - EPSILON
    )
    current_principal = 0.0 if completed else rounded(max(invested - principal_returned, 0))

    calculated_delay = max(
        [
            (today - date.fromisoformat(item["plannedDate"])).days
            for item in overdue_unpaid
            if item["plannedDate"]
        ] + [0]
    )
    # Istorinis vėlavimas nėra dabartinis vėlavimas. P3 naudojamas tik tada,
    # kai yra reali neapmokėta pradelsta įmoka.
    actual_delay_days = max(calculated_delay, sheet_delay if overdue_unpaid else 0)
...
def merge_overview(
    ...
    for investment in investments:
        ...
        for key in (
            "name", "country", "collateral", "term",
            "investmentDate", "plannedEndDate",
        ):
        ...
        if abs(investment["fees"]) <= EPSILON:
            investment["fees"] = overview["fees"]
        ...
        # Overview vėlavimas taikomas tik jei paskola nėra užbaigta ir
        # pačiame paskolos lape nustatyta neapmokėta pradelsta įmoka.
        has_unpaid_overdue = any(
            item["plannedDate"]
            and not item["paid"]
            and date.fromisoformat(item["plannedDate"]) < date.today()
            and item["principal"] + item["interest"] + item["fee"] > EPSILON
            for item in investment["schedule"]
        )
        if (
            investment["status"] != "completed"
            and has_unpaid_overdue
            and overview["delayDays"] > 0
        ):
            investment["status"] = "delayed"
            investment["delayDays"] = max(
                investment["delayDays"], overview["delayDays"]
            )
...
def build_history(
    ...
    for row in dated_flows:
        d = date.fromisoformat(row["date"])
        if row["amount"] > 0:
            monthly_income[(d.year, d.month)] += row["amount"]
    ...
    for period_end in month_range(start, end):
        ...
        history.append({
            ...
            "cash": 0.0,
            "income": rounded(received + money["bonus"]),
...
def build_distributions(
    ...
    def distribution(field: str) -> list[dict[str, Any]]:
        ...
        for item in active:
            ...
            grouped[label]["value"] = (
                float(grouped[label]["value"]) + item["currentValue"]
            )
    ...
    return {
        ...
        "borrower": distribution("borrower"),
        "collateral": distribution("collateral"),
        "loanType": distribution("loanType"),
...
def validate(data: dict[str, Any], expected_count: int) -> list[str]:
    ...
    if missing_top:
        errors.append("Trūksta laukų: " + ", ".join(sorted(missing_top)))
    ...
    if missing_summary:
        errors.append(
            "summary trūksta laukų: " + ", ".join(sorted(missing_summary))
        )

    if len(investments) != expected_count:
        errors.append(
            f"Paskolų skaičius nesutampa: Excel {expected_count}, "
            f"JSON {len(investments)}."
        )
    ...
    if abs(net_interest - rounded(summary.get("interestReceived"))) > 0.03:
        errors.append(
            f"Grynosios palūkanos nesutampa: paskolos {net_interest:.2f} "
            f"(bruto {total_interest:.2f} - mokesčiai {total_fees:.2f}), "
            f"Overview {rounded(summary.get('interestReceived')):.2f}."
        )

    total_principal = rounded(sum(item["principalReturned"] for item in investments))
    if abs(total_principal - rounded(summary.get("principalReturned"))) > 0.03:
        errors.append(
            f"Grąžinta paskola nesutampa: paskolos {total_principal:.2f}, "
            f"Overview {rounded(summary.get('principalReturned')):.2f}."
        )

    if abs(total_fees - rounded(summary.get("fees"))) > 0.03:
        errors.append(
            f"Mokesčiai nesutampa: paskolos {total_fees:.2f}, "
            f"Overview {rounded(summary.get('fees')):.2f}."
        )

    return errors


def load_lande(excel_file: Path) -> dict[str, Any]:
    ...
    try:
        ...
        outstanding = rounded(sum(item["currentValue"] for item in investments))
        active_value = sum(
            item["currentValue"]
        ...
        ltv_value = sum(
            item["currentValue"]
            for item in investments
            if item["status"] != "completed" and item.get("ltv") is not None
        )
        average_ltv = (
            rounded(
                sum(
                    item["ltv"] * item["currentValue"]
                    for item in investments
                    if item["status"] != "completed"
                    and item.get("ltv") is not None
                ) / ltv_value,
                4,
            )
            if ltv_value > EPSILON
            else None
        )
        ...
        summary = {
            ...
            "averageLtv": average_ltv,

            # Lande specifiniai laukai.
            "deposited": overview["deposited"],
            "withdrawn": withdrawn,
            "platformInvested": overview["currentPrincipal"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Lande Excel → Portfolio V2 platformos JSON."
    )
    parser.add_argument("--input", help="Lande Excel failo kelias.")
    parser.add_argument("--output", help="Lande JSON išvesties kelias.")
    ...
    output_file = (
        ...
        if args.output
        else project_root / "public" / "data" / "platforms" / "lande.json"
    )

    print("=" * 64)
    print("LANDE IMPORTER V1")
    print("=" * 64)
    print(f"Excel failas: {excel_file}")

    data = load_lande(excel_file)

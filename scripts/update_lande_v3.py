...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    if platform_col is None:
        raise ValueError(
            f"Faile {path.name} nerastas Lande stulpelių blokas."
        )

    # Kiekviena platforma pagrindiniame faile turi 3 stulpelius:
    # investuota / vertė / pokytis procentais.
    invested_col = platform_col
    value_col = platform_col + 1

    history: list[dict[str, Any]] = []

    for row in range(3, ws.max_row + 1):
        row_date = excel_date(ws.cell(row, 1).value)
        invested = money(ws.cell(row, invested_col).value)
        value = money(ws.cell(row, value_col).value)
    ...
    if not history:
        ...
        raise ValueError(
            ...
            f"Rastas platformos stulpelis: {platform_col}. "
            f"Patikrink, ar šiame bloke yra skaitinių reikšmių."
        )

"""Debitum P2P Excel failo importeris."""
...
def _read_overview_summary(ws) -> dict[str, float]:
    """Overview 3 eilutėje saugoma Debitum platformos suvestinė."""
                ...
                events.append(
                    {
                        ...
                        "description": {
                            "deposit": "Įnašas į Debitum",
                            "withdrawal": "Lėšų išėmimas iš Debitum",
                            "bonus": "Debitum premija",
                        }[event_type],
                    }
                )
...
def _read_loan_sheet(ws) -> dict[str, Any]:
    ...
    trust_score = str(ws["B6"].value or "").strip()
...
def _build_monthly_history(
    ...
    # Debitum Excel suvestinė yra galutinis autoritetas. Suvienodiname paskutinį tašką,
...
def load_debitum(source_file: str | Path) -> dict[str, Any]:
    source_path = Path(source_file)
    if not source_path.exists():
        raise FileNotFoundError(f"Debitum failas nerastas: {source_path}")

    workbook = load_workbook(source_path, data_only=True)
    if OVERVIEW_SHEET not in workbook.sheetnames or MONEY_SHEET not in workbook.sheetnames:
        raise ValueError("Debitum faile nerasti privalomi Overview ir Pinigai lapai.")

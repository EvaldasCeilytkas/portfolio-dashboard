...
EXCEL_PATH = ROOT / "excel" / "Lande.xlsx"
PORTFOLIO_PATH = ROOT / "public" / "data" / "portfolio.json"
PREVIEW_PATH = ROOT / "public" / "data" / "lande-platform.json"
MAIN_EXCEL_CANDIDATES = [
    ROOT / "excel" / "Investavimas.xlsx",
    *sorted((ROOT / "excel").glob("Investavimas*.xlsx")),
]
...
def read_loan(ws, overview: dict[str, Any], today: date) -> dict[str, Any]:
    ...
    borrower = str(find_value(ws, "Skolininkas") or "—")
    collateral = str(find_value(ws, "Įkeitimas") or overview.get("collateral") or "—")
    ltv = percent(find_value(ws, "LTV"))
...
def find_main_excel() -> Path:
    ...
    for candidate in MAIN_EXCEL_CANDIDATES:
        candidate = candidate.resolve()
        if candidate in seen:
            continue
        seen.add(candidate)

        if candidate.exists() and candidate.name.lower() != EXCEL_PATH.name.lower():
            return candidate
...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    workbook = load_workbook(path, data_only=True)

    if "Investavimas" not in workbook.sheetnames:
        raise KeyError(
            f"Faile {path.name} nerastas lapas „Investavimas“."
        )

    ws = workbook["Investavimas"]
    target_name = normalize_header("Lande")

    valid_invested = {"inesta", "investuota"}
    valid_value = {"verte"}
    valid_return = {"%", "pelningumas", "graza"}

    candidate_starts: list[int] = []

    # Pirmiausia ieškome tikro 3 stulpelių platformos bloko pagal 2 eilutę.
    for start_col in range(1, ws.max_column - 1):
        first = normalize_header(ws.cell(2, start_col).value)
        second = normalize_header(ws.cell(2, start_col + 1).value)
        third = normalize_header(ws.cell(2, start_col + 2).value)

        if (
            first in valid_invested
            and second in valid_value
            and third in valid_return
        ):
            # Platformos pavadinimas gali būti tame pačiame arba šalia esančiame
            # sujungtame 1 eilutės langelyje.
            nearby_headers = [
                normalize_header(ws.cell(1, col).value)
                for col in range(max(1, start_col - 2), min(ws.max_column, start_col + 2) + 1)
            ]

            if target_name in nearby_headers:
                candidate_starts.append(start_col)

    # Atsarginis variantas: ieškome pagal pavadinimą ir tikriname langelius aplink jį.
    if not candidate_starts:
        for name_col in range(1, ws.max_column + 1):
            if normalize_header(ws.cell(1, name_col).value) != target_name:
                continue

            for start_col in range(max(1, name_col - 3), min(ws.max_column - 2, name_col + 3) + 1):
                first = normalize_header(ws.cell(2, start_col).value)
                second = normalize_header(ws.cell(2, start_col + 1).value)
                third = normalize_header(ws.cell(2, start_col + 2).value)
    ...
    if not candidate_starts:
        raise ValueError(
            f"Faile {path.name} nerastas tikras Lande "
            "„Investuota / Vertė / %“ stulpelių blokas."
        )

    # Jei sutapo keli blokai, pasirenkame tą, kuriame yra realių skaitinių duomenų.
    selected_col = None
    selected_score = -1

    for start_col in candidate_starts:
        score = 0
        for row in range(3, ws.max_row + 1):
            invested_raw = ws.cell(row, start_col).value
            value_raw = ws.cell(row, start_col + 1).value
            if isinstance(invested_raw, (int, float)) and invested_raw != 0:
                score += 1
            if isinstance(value_raw, (int, float)) and value_raw != 0:
                score += 1
    ...
    if selected_col is None or selected_score <= 0:
        raise ValueError(
            f"Faile {path.name} rastas Lande blokas, "
            "bet jame nėra skaitinių istorijos reikšmių."
        )

    history: list[dict[str, Any]] = []

    for row in range(3, ws.max_row + 1):
        row_date = excel_date(ws.cell(row, 1).value)
        invested = money(ws.cell(row, selected_col).value)
        value = money(ws.cell(row, selected_col + 1).value)
    ...
    if not history:
        raise ValueError(
            f"Faile {path.name} nerasta Lande mėnesinės istorijos."
        )

    print(
        f"Lande pagrindinio failo stulpeliai: "
        f"{selected_col}–{selected_col + 2}"
    )
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    try:
        history = read_main_history(main_excel_path)
    except ValueError:
        # Platforma pradėta naudoti tik šį mėnesį, todėl pagrindiniame
        # faile dar nėra ankstesnės mėnesinės istorijos.
        history = [
            {
        ...
        print(
            "Lande mėnesinės istorijos pagrindiniame faile dar nėra. "
            "Naudojamas šio mėnesio pradinis taškas."
        )
    ...
    details = {
        ...
        "health": {
            ...
            "summary": (
                "Paskolos nevėluoja, tačiau portfelis yra labai koncentruotas."
                if not late
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            ...
            if item.get("slug") == "lande"
            or str(item.get("name", "")).strip().lower() == "lande"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-lande.json")
    ...
    print(f"Lande platforma {action}.")
    print(f"Atnaujintas failas: {PORTFOLIO_PATH}")
    print(f"Atsarginė kopija: {backup}")
    print("Atidaryk: http://localhost:5173/platforms/lande")
...
def main() -> None:
    if not EXCEL_PATH.exists():
        raise FileNotFoundError(
            f"Nerastas {EXCEL_PATH}. Įkelk Lande.xlsx į projekto excel aplanką."
        )

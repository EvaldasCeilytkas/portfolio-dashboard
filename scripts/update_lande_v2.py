...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    for column in range(1, ws.max_column + 1):
        heading = str(ws.cell(1, column).value or "").strip().lower()
        subheading = str(ws.cell(2, column).value or "").strip().lower()

        if heading == "lande" and subheading in {"įnešta", "inesta"}:
            invested_col = column
            break

    if invested_col is None:
        raise ValueError(
            f"Faile {path.name} nerasti Lande stulpeliai."
        )

    value_col = invested_col + 1
    history: list[dict[str, Any]] = []

    for row in range(3, ws.max_row + 1):
        row_date = excel_date(ws.cell(row, 1).value)
        invested = round_money(ws.cell(row, invested_col).value) if "lande" == "income" else money(ws.cell(row, invested_col).value)
        value = round_money(ws.cell(row, value_col).value) if "lande" == "income" else money(ws.cell(row, value_col).value)

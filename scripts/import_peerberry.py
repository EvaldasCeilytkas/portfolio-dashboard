...
def find_input(project_root: Path, explicit: str | None) -> Path:
    ...
    preferred = [
        excel_dir / "PeerBerry.xlsx",
        project_root / "PeerBerry.xlsx",
    ]
    for path in preferred:
        if path.is_file():
            return path

    candidates = sorted(excel_dir.glob("PeerBerry*.xlsx")) if excel_dir.exists() else []
    if candidates:
        return candidates[-1]

    raise FileNotFoundError(
        "Nerastas PeerBerry Excel failas. Įdėk jį į excel/PeerBerry.xlsx "
        "arba naudok --input."
    )
...
def load_peerberry(excel_file: Path) -> dict[str, Any]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PeerBerry Excel → Portfolio V2 platformos JSON."
    )
    parser.add_argument("--input", help="PeerBerry Excel failo kelias.")
    parser.add_argument("--output", help="PeerBerry JSON išvesties kelias.")
    ...
    output_file = (
        ...
        if args.output
        else project_root / "public" / "data" / "platforms" / "peerberry.json"
    )

    print("=" * 64)
    print("PEERBERRY IMPORTER V1")
    print("=" * 64)
    print(f"Excel failas: {excel_file}")

    data = load_peerberry(excel_file)

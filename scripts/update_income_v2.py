...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    for column in range(1, ws.max_column + 1):
        ...
        if heading == "income" and subheading in {"įnešta", "inesta"}:
            invested_col = column
            break

    if invested_col is None:
        raise ValueError(
            f"Faile {path.name} nerasti Income stulpeliai."
        )
    ...
    for row in range(3, ws.max_row + 1):
        ...
        invested = round_money(ws.cell(row, invested_col).value) if "income" == "income" else money(ws.cell(row, invested_col).value)
        value = round_money(ws.cell(row, value_col).value) if "income" == "income" else money(ws.cell(row, value_col).value)

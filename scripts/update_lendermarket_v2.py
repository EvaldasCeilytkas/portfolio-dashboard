...
EXCEL_PATH = ROOT / "excel" / "Lendermarket.xlsx"
PORTFOLIO_PATH = ROOT / "public" / "data" / "portfolio.json"
PREVIEW_PATH = ROOT / "public" / "data" / "lendermarket-platform.json"
...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    for column in range(1, ws.max_column + 1):
        ...
        if heading == "lendermarket" and subheading in {"įnešta", "inesta"}:
            invested_col = column
            break

    if invested_col is None:
        raise ValueError(
            f"Faile {path.name} nerasti Lendermarket stulpeliai."
        )
    ...
    if not history:
        raise ValueError(
            f"Faile {path.name} nerasta Lendermarket mėnesinės istorijos."
        )
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    for sheet_name in workbook.sheetnames:
        if sheet_name in {"Overview", "Pinigai"}:
            continue
        # Visi kiti lapai yra atskiros Lendermarket paskolos.
        loans.append(
            read_loan(
                workbook[sheet_name],
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            ...
            if item.get("slug") == "lendermarket"
            or str(item.get("name", "")).strip().lower() == "lendermarket"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-lendermarket.json")
    ...
    print(f"Lendermarket platforma {action}.")
    print(f"Atnaujintas failas: {PORTFOLIO_PATH}")
    print(f"Atsarginė kopija: {backup}")
    print("Atidaryk: http://localhost:5173/platforms/lendermarket")
...
def main() -> None:
    if not EXCEL_PATH.exists():
        raise FileNotFoundError(
            f"Nerastas {EXCEL_PATH}. Įkelk Lendermarket.xlsx į projekto excel aplanką."
        )

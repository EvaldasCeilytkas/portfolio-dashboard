...
EXCEL_PATH = ROOT / "excel" / "PeerBerry.xlsx"
...
PREVIEW_PATH = ROOT / "public" / "data" / "peerberry-platform.json"
...
def read_loan(ws, today: date) -> dict[str, Any]:
    ...
    originator = str(find_value(ws, "Skolintojas") or "—")
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    history = read_main_history(main_excel_path, "PeerBerry")

    if not history:
        raise ValueError(
            f"Faile {main_excel_path.name} nerasta PeerBerry mėnesinė istorija. "
            "Patikrink Investavimas lapo Peerberry antraštę ir stulpelius."
        )
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            ...
            if item.get("slug") == "peerberry"
            or str(item.get("name", "")).strip().lower() == "peerberry"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-peerberry.json")
    ...
    print(f"PeerBerry platforma {action}.")
    ...
    print("Atidaryk: http://localhost:5173/platforms/peerberry")

...
def find_input(project_root: Path, explicit: str | None) -> Path:
    ...
    for path in (excel_dir / "Afranga.xlsx", project_root / "Afranga.xlsx"):
        if path.is_file():
            return path

    candidates = sorted(excel_dir.glob("Afranga*.xlsx")) if excel_dir.exists() else []
    if candidates:
        return candidates[-1]

    raise FileNotFoundError(
        "Nerastas Afranga Excel failas. Įdėk jį į excel/Afranga.xlsx "
        "arba naudok --input."
    )
...
def read_overview(ws, epoch: datetime) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in range(3, ws.max_row + 1):
        ...
        rows.append({
            ...
            "lender": str(ws.cell(row, 5).value or "").strip() or None,
...
def load_afranga(excel_file: Path) -> dict[str, Any]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Afranga Excel → Portfolio V2 platformos JSON."
    )
    parser.add_argument("--input", help="Afranga Excel failo kelias.")
    parser.add_argument("--output", help="Afranga JSON išvesties kelias.")
    ...
    output_file = (
        ...
        if args.output
        else project_root / "public" / "data" / "platforms" / "afranga.json"
    )

    print("=" * 64)
    print("AFRANGA IMPORTER V1")
    print("=" * 64)
    print(f"Excel failas: {excel_file}")

    data = load_afranga(excel_file)

...
EXCEL_PATH = ROOT / "excel" / "Scramble.xlsx"
...
PREVIEW_PATH = ROOT / "public" / "data" / "scramble-platform.json"
...
def read_loan(ws, today: date) -> dict[str, Any]:
    ...
    originator = str(find_value(ws, "Skolintojas") or find_value(ws, "Kompanija") or "—")
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    history = read_main_history(main_excel_path, "Scramble")

    if not history:
        raise ValueError(
            f"Faile {main_excel_path.name} nerasta Scramble mėnesinė istorija. "
            "Patikrink Investavimas lapo Scramble antraštę ir stulpelius."
        )
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            ...
            if item.get("slug") == "scramble"
            or str(item.get("name", "")).strip().lower() == "scramble"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-scramble.json")
    ...
    print(f"Scramble platforma {action}.")
    ...
    print("Atidaryk: http://localhost:5173/platforms/scramble")

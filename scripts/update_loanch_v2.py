...
def read_main_history(path: Path) -> list[dict[str, Any]]:
    ...
    for column in range(1, ws.max_column + 1):
        ...
        if heading == "loanch" and subheading in {"įnešta", "inesta"}:
            invested_col = column
            break

    if invested_col is None:
        raise ValueError(
            f"Faile {path.name} nerasti Loanch stulpeliai."
        )
    ...
    for row in range(3, ws.max_row + 1):
        ...
        # Praleidžiame laikotarpius iki platformos naudojimo pradžios.
    ...
    if not history:
        raise ValueError(
            f"Faile {path.name} nerasta Loanch mėnesinės istorijos."
        )

...
EXCEL_PATH = ROOT / "excel" / "Viainvest.xlsx"
MAIN_EXCEL_PATH = ROOT / "excel" / "Investavimas.xlsx"


def find_main_excel() -> Path:
    """Randa pagrindinį investicijų failą, net jei jo pavadinime yra data."""
    if MAIN_EXCEL_PATH.exists():
        return MAIN_EXCEL_PATH

    candidates = sorted(
        (ROOT / "excel").glob("Investavimas*.xlsx"),
    ...
    raise FileNotFoundError(
        "Nerastas pagrindinis Investavimas.xlsx failas. "
        "Įkelk jį į projekto excel aplanką."
    )
PORTFOLIO_PATH = ROOT / "public" / "data" / "portfolio.json"
PREVIEW_PATH = ROOT / "public" / "data" / "viainvest-platform.json"
...
def read_overview(ws) -> tuple[list[dict[str, Any]], dict[str, float]]:
    ...
    for row in range(3, ws.max_row + 1):
        ...
        if sequence in (None, "") and name in (None, ""):
            continue
...
def read_payments(ws) -> list[dict[str, Any]]:
    ...
    for row in range(3, ws.max_row + 1):
        ...
        delay = int(number(ws.cell(row, 6).value))
        ...
        if planned and actual and delay == 0:
...
def read_loan(ws, today: date) -> dict[str, Any]:
    loan_id = str(find_value(ws, "Loan ID") or ws.title)
    unique_id = str(ws.title)
    country = str(find_value(ws, "Šalis") or "—")
    originator = str(find_value(ws, "Kompanija") or find_value(ws, "Skolintojas") or "—")
    loan_type = str(find_value(ws, "Tipas") or "Personal")
    term = str(find_value(ws, "Terminas") or "—")

    invested_date = excel_date(find_value(ws, "Investuota"))
    start_date = excel_date(find_value(ws, "Pradžia"))
    planned_end = excel_date(find_value(ws, "Pabaiga"))

    interest_rate = percent(find_value(ws, "Palūkanos"))
    invested = money(
        find_value(ws, "Investuota, Eur")
        or find_value(ws, "Investuota viso")
    )
    principal_repaid = money(ws.cell(3, 11).value)
    interest_received = money(ws.cell(3, 12).value)
    fees = money(ws.cell(3, 13).value)
    actual_end = excel_date(ws.cell(3, 15).value)
    delay_days = int(number(ws.cell(3, 16).value))
...
def read_main_history(path: Path, platform_name: str) -> list[dict[str, Any]]:
    """Nuskaito mėnesines įneštos sumos ir vertės reikšmes iš pagrindinio failo."""
    if not path.exists():
        return []
    ...
    for row in range(3, ws.max_row + 1):
        history_date = excel_date(ws.cell(row, 1).value)
        invested = money(ws.cell(row, target_col).value)
        value = money(ws.cell(row, target_col + 1).value)
...
def build_platform(path: Path) -> dict[str, Any]:
    ...
    _, summary = read_overview(workbook["Overview"])

    loans = []
    for sheet_name in workbook.sheetnames:
        if sheet_name in {"Overview", "Pinigai"}:
            continue
        loans.append(read_loan(workbook[sheet_name], today))
    ...
    profit = summary["profit"] or round(current_value - deposited, 2)
    ...
    performing = active + late
    performing_outstanding = sum(item["remainingPrincipal"] for item in performing)
    average_rate = (
        sum(item["interestRate"] * item["remainingPrincipal"] for item in performing)
        / performing_outstanding
        if performing_outstanding
        else 0
    )

    largest_share = (
        max((item["remainingPrincipal"] for item in performing), default=0)
        / current_value * 100
        if current_value
        else 0
    )
    ...
    for loan in performing:
        ...
        row = forecast.setdefault(
            month,
            {"month": month, "principal": 0.0, "interest": 0.0, "fees": 0.0, "total": 0.0},
        )
    ...
    # Grafiko istorija privalomai imama iš pagrindinio Investavimas failo.
    # Sąmoningai nebenaudojame „Pinigai“ lapo kaip atsarginio varianto,
    # nes jis nerodo tikro mėnesinio platformos vertės prieaugio.
    main_excel_path = find_main_excel()
    history = read_main_history(main_excel_path, "Viainvest")

    if not history:
        raise ValueError(
            f"Faile {main_excel_path.name} nerasta Viainvest mėnesinė istorija. "
            "Patikrink Investavimas lapo Viainvest antraštę ir stulpelius."
        )

    # Jei platformos failas jau atnaujintas naujam mėnesiui, o pagrindiniame
    # Investavimas faile dar yra tik praėjusio mėnesio eilutė, pridedame
    # dabartinį tašką iš PeerBerry suvestinės.
    current_month = today.strftime("%Y-%m")
    if history[-1]["date"][:7] != current_month:
        history.append(
            {
        ...
        history[-1]["invested"] = deposited
        history[-1]["value"] = current_value

    diversification_score = min(20, 5 + len(performing))
    quality_score = max(0, round(20 - late_share))
    cash_score = max(0, min(20, round(20 - max(0, cash_share) * 0.6)))
    repayment_score = 16 if not income_history else 19
    concentration_score = max(0, round(20 - max(0, largest_share - 20) * 0.13))
    health_score = int(
        diversification_score + quality_score + cash_score
        + repayment_score + concentration_score
    )
    ...
    details = {
        ...
        "cashflow": {
            ...
            "netIncome": money(sum(item["netIncome"] for item in loans) + bonuses),
        },
        "health": {
            "score": health_score,
            "label": (
                "Puiki būklė" if health_score >= 85
                else "Gera būklė" if health_score >= 70
                else "Reikia stebėti"
            ),
            "summary": (
                "Portfelis diversifikuotas, tačiau yra vėluojančių paskolų."
                if late
                else "Paskolos nevėluoja ir portfelis yra diversifikuotas."
            ),
            ...
            "metrics": [
                {
                    ...
                    "max": 20,
                    "description": f"Portfelyje yra {len(loans)} paskolos.",
                },
                {
                    "key": "loanQuality",
                ...
                },
                {
                    ...
                    "max": 20,
                    "description": "Vertinama pagal faktinių grąžinimų istoriją.",
                },
                {
                    "key": "concentration",
                    ...
                    "max": 20,
                    "description": f"Didžiausia paskola sudaro {largest_share:.1f} % portfelio.",
                },
            ],
        },
    ...
    return {
        ...
        "analytics": {
            ...
            "highestValue": max([number(item["value"]) for item in history], default=current_value),
...
def merge_into_portfolio(platform: dict[str, Any]) -> None:
    ...
    index = next(
        (
            i for i, item in enumerate(platforms)
            if item.get("slug") == "viainvest"
            or str(item.get("name", "")).strip().lower() == "viainvest"
        ),
        None,
    )
    ...
    backup = PORTFOLIO_PATH.with_suffix(".before-viainvest.json")
    ...
    print(f"Viainvest platforma {action}.")
    print(f"Grafiko istorijos taškų: {len(platform.get('history', []))}")
    if platform.get("history"):
        print(f"Pirmas taškas: {platform['history'][0]}")
        print(f"Paskutinis taškas: {platform['history'][-1]}")
    print(f"Atnaujintas failas: {PORTFOLIO_PATH}")
    print(f"Atsarginė kopija: {backup}")
    print("Atidaryk: http://localhost:5173/platforms/viainvest")


def main() -> None:
    if not EXCEL_PATH.exists():
        raise FileNotFoundError(
            f"Nerastas {EXCEL_PATH}. Įkelk Nectaro.xlsx į projekto excel aplanką."
        )
